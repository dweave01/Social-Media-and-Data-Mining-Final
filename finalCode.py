...
def get_user_profile(twitter_api, screen_names=None, user_ids=None):
    ...
    return response
############################################################ My function, used to get 3 most popular users 
def get_three_best(screen_name=None, user_id=None):
    ...
    new=[]
    for thing in people: #this loop obtains the id and followers count of each user 
        a=(thing['id'])
    ...
    threeBest=[val[1] for val in new] #take ids only from the pairs
    final=threeBest[:3] #keep only the top 3
...
final=get_three_best(screen_name=screen_name) #run get five best on the starter
everyone= []
everyone.append(og)
...
for unit in toDo: #for all to do list items
    if len(toDo)<=nodeNum: #while 100 nodes have not been collected 
        ...
        try:
            here=(get_three_best(user_id=unit)) #run 5 best on currently looked at user
...
bad_words= ["this", "that", "have", "just", "with", "where", "your", "when", "ever", "dont't", "don’t","dont","like","what","will","they",
            "here","their","&amp","other","think","from", "about"] #non helpful words are filtered out
ending=[]
